...
const DoctorDeleteModal = ({ refetch,doctor }) => {
    const {email,name}=doctor

    const hendelDelet = () => {
        fetch(`https://doctors-portal-server-dufg.onrender.com/doctor/${email}`, {
            method: 'DELETE',
            headers: {
            ...
            .then(data => {
                if (data.deletedCount) {
                    toast.success(`successfully remove an Doctor ${name} `)
                    refetch();
                }
            })
    } 
        ...
        <div>
            ...
            <div className="modal" role="dialog">
                <div className="modal-box">
                    <h3 className="text-lg font-bold capitalize text-error">are you sure you want to delete doctor {name}?</h3>
